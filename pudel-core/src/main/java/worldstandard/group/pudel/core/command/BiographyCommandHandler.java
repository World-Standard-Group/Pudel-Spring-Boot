...
/**
 * Handler for the biography command.
 * Sets Pudel's biography for this guild.
 */
@Component
public class BiographyCommandHandler implements TextCommandHandler {

    private final GuildInitializationService guildInitializationService;

    public BiographyCommandHandler(GuildInitializationService guildInitializationService) {
    ...
    public void handle(CommandContext context) {
        ...
        if (context.getArgs().length == 0) {
            String bio = settings.getBiography() != null ? settings.getBiography() : "Not set";
            context.getChannel().sendMessage("Current biography:\n" + bio + "\n\n" +
                    "Usage: `!biography <text>`").queue();
            return;
        }

        String biography = String.join(" ", context.getArgs());
        settings.setBiography(biography);
        guildInitializationService.updateGuildSettings(context.getGuild().getId(), settings);

        context.getChannel().sendMessage("✅ Biography updated!").queue();
    }
}

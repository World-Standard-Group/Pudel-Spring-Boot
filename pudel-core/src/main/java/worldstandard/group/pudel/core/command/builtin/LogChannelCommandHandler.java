...
/**
 * Handler for the logchannel command.
 * Sets the channel where command logs should be sent.
 */
@Component
public class LogChannelCommandHandler implements TextCommandHandler {

    private final GuildInitializationService guildInitializationService;

    public LogChannelCommandHandler(GuildInitializationService guildInitializationService) {
    ...
    public void handle(CommandContext context) {
        ...
        if (context.getArgs().length < 1) {
            String currentChannel = settings.getLogChannel() != null ? "<#" + settings.getLogChannel() + ">" : "Not set";
            context.getChannel().sendMessage("Current log channel: " + currentChannel + "\n" +
                    "Usage: `!logchannel <#channel>` or `!logchannel none`").queue();
            return;
        }

        if (context.getArgs()[0].equalsIgnoreCase("none")) {
        ...
        if (context.getMessage().getMentions().getChannels().isEmpty()) {
            context.getChannel().sendMessage("❌ Please mention a channel: `!logchannel #channel`").queue();
            return;
        }
